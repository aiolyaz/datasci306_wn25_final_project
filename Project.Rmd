...
```{r}
# Load in tables
# If this errors, make sure you run the code in data/Preprocessing.Rmd first
...
Character columns
  - **nconst** - unique identifier, not factor
  - **primaryName** - name of person, not factor
  - **primaryProfession** - list of professions, can be split up and used as factor
  - **knownForTitles** - looks like random identifiers, not factor

```{r}
# Distribution for primaryProfession
name_basics |>
  filter(!is.na(primaryProfession)) |>
  pull(primaryProfession) |>
  strsplit(",") |>
  unlist() |>
  table()
```
...
title_principals |>
  select(where(is.numeric)) |>
  summary()
sapply(title_principals, n_distinct)
unique(title_principals$category)
```

Columns:
- `tconst` - Used similarly to a primary key, used to uniquely identify the title
- `ordering` - an integer used to further identify more specific than just the title id from `tconst`
- `nconst` - Used to uniquely identify a person
- `category` - Only factor in this table, used to categorize their role in the title
- `job` - Further specifies what their role was in the title
- `characters` - Name of the character played in the title (if applicable)


```{r}
ggplot(title_principals, aes(x = category)) +
  geom_bar() +
  labs(title = "Number of people in each role category for titles in the IMDb", x = "Category", y = "Count") +
  theme_minimal() +
...
```{r}
capture_string1 = "(^Tom )|(^Thomas )|(^Thom )|(^Tomas )"
filter(name_basics, str_detect(primaryName, capture_string1))
```

* How many titles use alliteration (i.e., all words in the title start with the same letter)?

```{r}
capture_string2 = "^([a-zA-Z])[a-zA-Z]* (\\1([a-zA-Z])* )*(\\1([a-zA-Z])*)$"
filter(title_basics, str_detect(primaryTitle, capture_string2))
```
...
rate_count_year = basics_ratings |>
  filter(!is.na(numVotes) & !is.na(startYear)) |>
  group_by(startYear) |>
  summarize(total_votes = sum(numVotes))
# Still need to plot and show numerical evidence
```
...
```{r}
rate_principals = inner_join(title_principals, title_ratings, by = "tconst")
top_principals = rate_principals |>
  group_by(nconst) |>
  summarize(median_rating = median(averageRating)) |>
  arrange(desc(median_rating)) |>
  head(10) |>
  select(nconst)
inner_join(top_principals, name_basics, by = "nconst") |>
  select(primaryName)
# This only does the first bullet point and the 2nd, need to find proportions and graph rating against years
